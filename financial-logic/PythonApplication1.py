...
for index, row in inc.iterrows():
	...
	if(Reported.inc.YEAR_INC == label ):
		...
		data['NI_AVAIL_COMMON_GAAP'] = Reported.ticker.NI_AVAIL_COMMON_GAAP.to_json(orient='records')
		...
		data['NI_AVAIL_COMMON_ADJ'] = Reported.ticker.NI_AVAIL_COMMON_ADJ.to_json(orient='records')
	elif(Reported.inc.NET_ABNORMAL == label ):
		Reported.ticker.NET_ABNORMAL = row
		Reported.ticker.NET_ABNORMAL[0] = label
		data['NET_ABNORMAL'] = Reported.ticker.NET_ABNORMAL.to_json(orient='records')
	elif(Reported.inc.NET_EXTRA2 == label ):
		Reported.ticker.NET_EXTRA2 = row
		Reported.ticker.NET_EXTRA2[0] = label
		data['NET_EXTRA2'] = Reported.ticker.NET_EXTRA2.to_json(orient='records')
		...
		data['BASIC_WEIGHT_AVG_SHARES'] = Reported.ticker.BASIC_WEIGHT_AVG_SHARES.to_json(orient='records')
		...
		data['DIL_WEIGHT_AVG_SHARES'] = Reported.ticker.DIL_WEIGHT_AVG_SHARES.to_json(orient='records')
	else:
		unkept.append(label)
...
for index, row in bal.iterrows():
	...
	if(Reported.bal.YEAR_BAL == label ):
		...
		data['DERIV_HEDGE_ASSETS2'] = Reported.ticker.DERIV_HEDGE_ASSETS2.to_json(orient='records')
	elif(Reported.bal.MISC_ASSETS == label ):
		Reported.ticker.MISC_ASSETS = row
		Reported.ticker.MISC_ASSETS[0] = label
		data['MISC_ASSETS'] = Reported.ticker.MISC_ASSETS.to_json(orient='records')
		...
		data['TOTAL_NON_CURR_LIAB'] = Reported.ticker.TOTAL_NON_CURR_LIAB.to_json(orient='records')
	elif(Reported.bal.TOTAL_LIAB == label ):
		Reported.ticker.TOTAL_LIAB = row
		Reported.ticker.TOTAL_LIAB[0] = label
		data['TOTAL_LIAB'] = Reported.ticker.TOTAL_LIAB.to_json(orient='records')
		...
		data['EQUITY_BEFORE_MIN_INT'] = Reported.ticker.EQUITY_BEFORE_MIN_INT.to_json(orient='records')
		...
		data['MIN_NON_CONTROL_INT'] = Reported.ticker.MIN_NON_CONTROL_INT.to_json(orient='records')
	elif(Reported.bal.TOTAL_EQUITY == label ):
		Reported.ticker.TOTAL_EQUITY = row
		Reported.ticker.TOTAL_EQUITY[0] = label
		data['TOTAL_EQUITY'] = Reported.ticker.TOTAL_EQUITY.to_json(orient='records')
	elif(Reported.bal.LIAB_AND_EQUITY2 == label ):
		Reported.ticker.LIAB_AND_EQUITY2 = row
		Reported.ticker.LIAB_AND_EQUITY2[0] = label
		data['LIAB_AND_EQUITY2'] = Reported.ticker.LIAB_AND_EQUITY2.to_json(orient='records')
	else:
		unkept.append(label)
...
for index, row in cf.iterrows():
	...
	if(Reported.cf.YEAR_CF == label ):
		...
		data['DISP_FIXED_PROD_ASSETS'] = Reported.ticker.DISP_FIXED_PROD_ASSETS.to_json(orient='records')
	elif(Reported.cf.DISP_INTAG_ASSETS == label ):
		Reported.ticker.DISP_INTAG_ASSETS = row
		Reported.ticker.DISP_INTAG_ASSETS[0] = label
		data['DISP_INTAG_ASSETS'] = Reported.ticker.DISP_INTAG_ASSETS.to_json(orient='records')
	elif(Reported.cf.ACQ_FIXED_INTAG == label ):
		Reported.ticker.ACQ_FIXED_INTAG = row
		Reported.ticker.ACQ_FIXED_INTAG[0] = label
		data['ACQ_FIXED_INTAG'] = Reported.ticker.ACQ_FIXED_INTAG.to_json(orient='records')
		...
		data['NET_CASH_DISC_OPS3'] = Reported.ticker.NET_CASH_DISC_OPS3.to_json(orient='records')
	elif(Reported.cf.CASH_FIN_ACT2 == label ):
		Reported.ticker.CASH_FIN_ACT2 = row
		Reported.ticker.CASH_FIN_ACT2[0] = label
		data['CASH_FIN_ACT2'] = Reported.ticker.CASH_FIN_ACT2.to_json(orient='records')
