...
GROWTH_MULTIPLE = 2
REQUIRED_RETURN = 4.4
AAA_BOND_YIELD = 3.04 #Recorded on 10/19/2019
...
class Calc_Names:
	...
	FIXED_ASSET_TURN_RATE			= "Fixed Asset Turnover Rate in Days"
	...
	EPS_DILUTED						= "Earnings per Share on Diluted Shares"
	...
	INTRINSIC_VALUE_UNFCF			= "Intrinsic Value by Unlevered Free Cash Flow"
	...
	MARGIN_OF_SAFETY_UNFCF			= "Margin of Safety by Unlevered Free Cash Flow"
...
class Calc_Formulas:
	...
	AVG_PAYABLES					= "1/2 (Beginning + Ending) Accounts Payable"
	...
	AVG_FIXED_ASSETS				= "1/2 (Beginning + Ending) Fixed Assets"
	...
	FIXED_ASSET_TURNOVER			= "Sales / Average Net Fixed Assets"
	FIXED_ASSET_TURN_RATE			= "Days Fixed Asset * 365"
	...
	EPS_DILUTED						= "Net Income / Diluted Shares Outstanding"
	...
	INTRINSIC_VALUE_UNFCF			= "Unlevered Free Cash Flow Average (ith year) / (1 + Discount rate) ^ (ith year)"
	...
	MARGIN_OF_SAFETY_UNFCF			= "Intrinsic Value (using UnLevered Free Cash Flow) / Market Capitalization"
...
class Calc_Vars:
	...
	FIXED_ASSET_TURNOVER			= [None] * 35
	FIXED_ASSET_TURN_RATE			= [None] * 35
...
class Calc_Averages:
	def avgAcctsReceivable(ACCTS_REC_0, ACCTS_REC_1):
		return ((ACCTS_REC_0 + ACCTS_REC_1) / 2) if (ACCTS_REC_0 != None and ACCTS_REC_1 != None) else None
	def avgAcctsPayable(PAYABLES_0, PAYABLES_1):
		return ((PAYABLES_0 + PAYABLES_1) / 2) if (PAYABLES_0 != None and PAYABLES_1 != None) else None
	def avgWorkingCapital(WORKING_CAPITAL_0, WORKING_CAPITAL_1):
		return ((WORKING_CAPITAL_0 + WORKING_CAPITAL_1) / 2) if (WORKING_CAPITAL_0 != None and WORKING_CAPITAL_1 != None) else None
	def avgInventory(INV_0, INV_1):
		return ((INV_0 + INV_1) / 2) if (INV_0 != None and INV_1 != None) else None
	def avgInvestments(TOTAL_INVEST_0, TOTAL_INVEST_1):
		return ((TOTAL_INVEST_0 + TOTAL_INVEST_1) / 2) if (TOTAL_INVEST_0 != None and TOTAL_INVEST_1 != None) else None
	def avgFixedAssets(CHG_FIXED_INTANG_0, CHG_FIXED_INTANG_1):
		return ((CHG_FIXED_INTANG_0 + CHG_FIXED_INTANG_1) / 2) if (CHG_FIXED_INTANG_0 != None and CHG_FIXED_INTANG_1 != None) else None
	def avgAssets(TOTAL_ASSETS1_0, TOTAL_ASSETS1_1):
		return ((TOTAL_ASSETS1_0 + TOTAL_ASSETS1_1) / 2) if (TOTAL_ASSETS1_0 != None and TOTAL_ASSETS1_1 != None) else None
	def avgLiabilities(TOTAL_LIAB_0, TOTAL_LIAB_1):
		return ((TOTAL_LIAB_0 + TOTAL_LIAB_1) / 2) if (TOTAL_LIAB_0 != None and TOTAL_LIAB_1 != None) else None
	def avgEquity(TOTAL_EQUITY_0, TOTAL_EQUITY_1):
		return ((TOTAL_EQUITY_0 + TOTAL_EQUITY_1) / 2) if (TOTAL_EQUITY_0 != None and TOTAL_EQUITY_1 != None) else None
	def avgDebt(TOTAL_DEBT_0, TOTAL_DEBT_1):
		return ((TOTAL_DEBT_0 + TOTAL_DEBT_1) / 2) if (TOTAL_DEBT_0 != None and TOTAL_DEBT_1 != None) else None
...
class Calc_Solvency: 
	...
	def quickRatio_2(TOTAL_CURR_ASSETS, INV, PREPAID_EXP, TOTAL_CURR_LIAB):
...
class  Calc_CapStructure:
	...
	def degreeCombinedLeverage(EPS_DILUTED_0, EPS_DILUTED_1, REV_0, REV_1):
		return round(((EPS_DILUTED_0 - EPS_DILUTED_1) / EPS_DILUTED_1) / ((REV_0 - REV_1) / REV_1), 2) if (EPS_DILUTED_0 != None and EPS_DILUTED_1 != None and EPS_DILUTED_1 != 0 and REV_0 != None and REV_1 != None and REV_1 != 0) else None
	...
	def degreeFinancialLeverage(EPS_DILUTED_0, EPS_DILUTED_1, EBIT_0, EBIT_1):
		return round(((EPS_DILUTED_0 - EPS_DILUTED_1) / EPS_DILUTED_1) / ((EBIT_0 - EBIT_1) / EBIT_1), 2) if (EPS_DILUTED_1 != None and EPS_DILUTED_1 != None and EPS_DILUTED_1 != 0 and EBIT_0 != None and EBIT_1 != None and EBIT_1 != 0) else None
	...
	def netAssetValue(TOTAL_ASSETS1, TOTAL_LIAB, EPS_DILUTED):
		return ((TOTAL_ASSETS1 - TOTAL_LIAB) / EPS_DILUTED) if (TOTAL_ASSETS1 != None and TOTAL_LIAB != None and EPS_DILUTED != None and EPS_DILUTED != 0) else None
	...
	def grahamFairReturnRate():
		return 1 / 8.5
	def wacc(TOTAL_EQUITY, TOTAL_DEBT, FAIR_RETURN_RATE, EFFECTIVE_INT_RATE, TAX_RATE ):
		return	round((TOTAL_EQUITY / (TOTAL_DEBT + TOTAL_EQUITY) * FAIR_RETURN_RATE) + (TOTAL_DEBT / (TOTAL_DEBT + TOTAL_EQUITY) * EFFECTIVE_INT_RATE * (1 - TAX_RATE)), 2) if(TOTAL_EQUITY != None and TOTAL_DEBT != None and FAIR_RETURN_RATE != None and EFFECTIVE_INT_RATE != None and TAX_RATE != None and (TOTAL_DEBT + TOTAL_EQUITY) != 0 ) else None
	
class  Calc_Asset_Activity:
	...
	def fixedAssetTurnover(REV, AVG_FIXED_ASSETS):
		return round((REV / AVG_FIXED_ASSETS), 2) if (REV != None and AVG_FIXED_ASSETS != None and AVG_FIXED_ASSETS != 0) else None
	def fixedAssetTurnoverRate(FIXED_ASSET_TURNOVER):
		return round((365 * FIXED_ASSET_TURNOVER), 2) if (FIXED_ASSET_TURNOVER != None) else None
...
class  Calc_Liab_Activity:
	...
	def liabitiesTurnover(TOTAL_LIAB, AVG_PAYABLES):
		return round((TOTAL_LIAB / AVG_PAYABLES), 2) if (TOTAL_LIAB != None and AVG_PAYABLES != None and AVG_PAYABLES != 0) else None
...
class  Calc_Profitability:
	...
	def epsDiluted(NI_INC, DIL_WEIGHT_AVG_SHARES):
	...
	def netOperatingProfitAfterTaxEBIT(OP_INC_LOSS, TAX_RATE):
		return round((OP_INC_LOSS * (1 - TAX_RATE)), 2) if (OP_INC_LOSS != None and TAX_RATE != None) else None
...
class  Calc_Valuations:
	...
	def originalGraham(EPS_DILUTED, NO_GROWTH_PE, GROWTH_MULTIPLE, GROWTH_RATE):
		return round((EPS_DILUTED (NO_GROWTH_PE + GROWTH_MULTIPLE * GROWTH_RATE)), 2) if (EPS_DILUTED != None and NO_GROWTH_PE != None and GROWTH_MULTIPLE != None and GROWTH_RATE != None) else None
	def revisedGraham(EPS_DILUTED, NO_GROWTH_PE, GROWTH_MULTIPLE, GROWTH_RATE, REQUIRED_RETURN, AAA_BOND_YIELD):
		return round(((EPS_DILUTED * (NO_GROWTH_PE + GROWTH_MULTIPLE * GROWTH_RATE) * REQUIRED_RETURN ) / AAA_BOND_YIELD), 2) if (EPS_DILUTED != None and NO_GROWTH_PE != None and GROWTH_MULTIPLE != None and GROWTH_RATE != None and MIN_FAIR_RETURN != None and AAA_BOND_YIELD != None and AAA_BOND_YIELD != 0) else None
	...
	def intrinsicValueNI(VALUE, NI_3YEAR_AVG, DISC_RATE, i):
		return None if ((VALUE == None or NI_3YEAR_AVG == None or DISC_RATE == None or i == None or ((1 + DISC_RATE) ** i)) == 0)
	...
	def intrinsicValueEBIT(VALUE, EBIT_3YEAR_AVG, DISC_RATE, i):
		return None if ((VALUE == None or EBIT_3YEAR_AVG == None or DISC_RATE == None or i == None or ((1 + DISC_RATE) ** i)) == 0)
		#print(str((i-1)) + ": " + str(VALUE))
		VALUE = (VALUE + (EBIT_3YEAR_AVG / ((1 + DISC_RATE) ** i)) )
		return VALUE if (EBIT_3YEAR_AVG / ((1 + DISC_RATE) ** i) < 1) else intrinsicValueNI(VALUE, EBIT_3YEAR_AVG, DISC_RATE, i+1)
	def intrinsicValueFCF(VALUE, FCF_3YEAR_AVG, DISC_RATE, i):
		return None if ((VALUE == None or FCF_3YEAR_AVG == None or DISC_RATE == None or i == None or ((1 + DISC_RATE) ** i)) == 0)
		#print(str((i-1)) + ": " + str(VALUE))
		VALUE = (VALUE + (FCF_3YEAR_AVG / ((1 + DISC_RATE) ** i)) )
		return VALUE if (FCF_3YEAR_AVG / ((1 + DISC_RATE) ** i) < 1) else intrinsicValueNI(VALUE, FCF_3YEAR_AVG, DISC_RATE, i+1)
	...
	def marginOfSafety_EBIT(IVALUE_UNFCF, MV):
		return round((1 - (IVALUE_UNFCF / MV)) * 100, 2) if (IVALUE_UNFCF != None and MV != None and MV != 0) else None
	def marginOfSafety_EBIT(IVALUE_FCF, MV):
...
#*************************************
	...
	#MRP							= "Return of Market - Risk Free Return"
	#MRP							= "Market Risk Premium"
	#RADR							= ""
	#RADR							= "Risk Adjusted Discount Rate"
	#YTM							= "Yield to Maturity"
	#YTM							= ""
	#RISK_FREE_RETURN				= "Rate of a Risk Free Investment"
	#RISK_FREE_RETURN				= ""
	#EQUITY_COST_CAP				= "Equity Cost of Capital"
	#EQUITY_COST_CAP				= ""

	
	##Calculations for ticker Object:
	#DCF							= "Discounted Cash Flow"
	#DCF							= ""
	#NPV							= "Net Present Value"
	#NPV_1							= "Sum[ (Net Cash Inflows - Outflows) / (1 + WACC) ^ Number of periods ]"
	#NPV_2							= "Today's Values of Expected Cash - Today's Value of Invested Cash"